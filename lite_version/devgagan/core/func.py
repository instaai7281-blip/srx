# ---------------------------------------------------
# File Name: func.py (Lite Version)
# Description: Minimal core functions for content extraction.
# ---------------------------------------------------
...
async def chk_user(message, user_id):
    # Lite version: No premium checks
    return 0
...
async def progress_bar(current, total, ud_type, message, start):
    ...
    if round(diff % 10.00) == 0 or current == total:
        ...
        try:
            await message.edit(text="{}    {}".format(ud_type, tmp))
        except:
            pass
...
async def userbot_join(userbot, invite_link):
    try:
        ...
        return "Could not join. Link expired or Invalid."
    except FloodWait:
...
async def progress_callback(current, total, progress_message):
    ...
    if current_time - last_update_time >= 10 or percent % 10 == 0:
        ...
        pb = "❤️" * completed_blocks + "🤍" * remaining_blocks
